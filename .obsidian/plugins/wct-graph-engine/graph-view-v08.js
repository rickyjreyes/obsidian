...
WCTGraphView.prototype.installV07Controls = function installV09Controls() {
...
function applyPriorityTypePolicy(graph) {
  const factors = {
    Equations: 1.18, Derivations: 1.22, Claims: 1.18, Theorems: 1.15,
    Contradictions: 1.2, Predictions: 1.24, Experiments: 1.24, Glossary: 1,
    Papers: 0.9, Repositories: 0.88, References: 0.48, Artifacts: 0.4, Other: 0.62,
  };
  for (const node of graph.nodes) {
    const profile = node.priorityProfile;
    if (!profile) continue;
    profile.rawScore = profile.score;
    profile.score = Math.round(Math.max(profile.explicit ?? 0, Math.min(100, profile.score * (factors[node.type] ?? 0.8))));
  }
  graph.priorityNodes = [...graph.nodes].sort((a, b) => (b.priorityProfile?.score ?? 0) - (a.priorityProfile?.score ?? 0)
    || (b.degree ?? 0) - (a.degree ?? 0)
    || a.label.localeCompare(b.label));
  assignPriorityRanks(graph);
}

WCTGraphView.prototype.rebuildGraph = async function rebuildGraphV09() {
...
WCTGraphView.prototype.showFull = function showFullV09(force = false) {
...
WCTGraphView.prototype.pushCategory = function pushCategoryV09(type, originNode = null) {
...
WCTGraphView.prototype.pushConnections = function pushConnectionsV09(nodeId, originNode = null) {
